            ...
            <tbody className="border border-white m-4">
              <tr>
                ...
                {weekDays.map((day, index) => (
                  <td
                    key={index}
                    className="border border-white text-center bg-blue-950 font-bold py-2 px-3"
                  >
                    {day}
                  </td>
                ))}
              ...
              {employees.map((employee, index) => (
                <tr key={index} className="border border-white">
                  <td
                    key={index}
                    className="border border-white p-1 bg-blue-950"
                  >
                    {employee.firstName} {employee.lastName}
                  </td>
                  {weekDays.map((day, index) => {
                    const userShifts = shifts.filter(
                      (shift) => shift.employee_id === employee._id
                    );
                    const weekdayShifts = userShifts.filter(
                      (shift) => new Date(shift.startDate).getDay() === index
                    );
                    if (weekdayShifts.length > 0) {
                      return (
                        <td
                          ...
                          className="border border-blue-950 p-1 bg-blue-50 text-black text-center px-5"
                        >
                          {weekdayShifts[0]._id ? "🟢" : ""}
                        </td>
                      );
